...
- (void)setModel:(YUUPendingMailboxModel *)model {
    ...
    if (model.sellorbuy == 1) { // 买家
        if (model.progressnum == 1) { // 发起交易
            ...
            _contentLabel.text = [NSString stringWithFormat:@"支付%ld转入%ldYUU，总价%@元，等待卖方二次确认",(long)model.sorbid,(long)model.coinnum, regYUUCoin([NSNumber numberWithDouble:model.price], 2)];
        }
    } else {
        if (model.progressnum == 1) { // 发起交易
            _middleBtn.hidden = NO;
            ...
            _contentLabel.text = [NSString stringWithFormat:@"收到转给%ld，%ldYUU，总价%@元，等待您的二次确认",(long)model.sorbid,(long)model.coinnum, regYUUCoin([NSNumber numberWithDouble:model.price], 2)];
        }
    }
